const statusMap = {
  ...
  confirmed: 'bg-[var(--trust-green)]/10 text-[var(--trust-green)] border-[var(--trust-green)]',
  rejected: 'bg-[var(--seal)]/10 text-[var(--seal)] border-[var(--seal)]',
  modified: 'bg-[var(--alert-amber)]/20 text-[var(--ink)] border-[var(--alert-amber)]',
  pending: 'bg-[var(--alert-amber)]/20 text-[var(--alert-amber)] border-[var(--alert-amber)]',
  'needs-changes': 'bg-[var(--seal)]/10 text-[var(--seal)] border-[var(--seal)]'
}

export default function StatusBadge({ status }) {
  const normalized = String(status || 'draft').toLowerCase().replace(/\s+/g, '-')
  return (
    <span className={`inline-flex -rotate-2 items-center border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${statusMap[normalized] || statusMap.draft}`}>
      {normalized}
    </span>
  )
}
